...
run = wandb.init(
    name = "baseline_model_no_dropout_pretrain", ## Wandb creates random run names if you skip this field
...
mfcc_list = ["mfcc_441", "mfcc_882", "mfcc_1323"]
# mfcc_list = ["mfcc_160"]

class AudioDataset(torch.utils.data.Dataset):

    def __init__(self, data_path, context, offset=0, partition= "train", limit=-1, shuffle=True, Normalization=True, mfcc_list=mfcc_list): # Feel free to add more arguments
        ...
        for index, mfcc_file in enumerate(mfcc_list):
            if partition == "train":
                self.mfcc_dir = data_path + '/' + f"train_{mfcc_file}" + "/"
                self.transcript_dir = data_path + "/train_processed_labels/"
            else:
                self.mfcc_dir = data_path + '/' + f"test_{mfcc_file}" + "/"
                self.transcript_dir = data_path + "/test_processed_labels/"
...
val_data = AudioDataset(data_path, context = config['context'], offset=0, partition= "test", limit=-1, shuffle=False) 
...
for epoch in range(config['epochs']):
    ...
    if eval_accuracy >= best_acc:
        ...
        torch.save({'epoch': epoch,
                ...
                'f1_score': eval_f1,
                'recall': eval_recall}, 
            './model_checkpoint_base_pretrain.pth'
        )
